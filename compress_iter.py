...
def main(args):
    ...
    mask_shape = [
        video_shape[0],
        1,
        video_shape[2] // args.tile_size,
        video_shape[3] // args.tile_size,
    ]
    ...
    for iteration in range(args.num_iterations + 1):
        ...
        for fid, (video_slices, mask_slice) in enumerate(
            ...
            if fid % 5 == 0 and (iteration + 1) % (args.num_iterations // 4) == 0:
                heat = tile_mask(mask[fid : fid + 1, :, :, :], args.tile_size)[
                    0, 0, :, :
                ]
                plt.clf()
                ...
                image = application.plot_results_on(
                    inference_results, image, (0, 255, 255), args
                )
