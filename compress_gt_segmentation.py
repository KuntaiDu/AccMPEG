...
def main(args):
    ...
    ground_truth_dict = read_results(args.ground_truth, application.name, logger)
    ...
    for temp in range(1):
        ...
        for fid, (video_slices, mask_slice) in enumerate(
            ...
            with torch.no_grad():
                gt_results = ground_truth_dict[fid][None, :, :]

                gt_results = torch.where(
                    gt_results > 0,
                    torch.ones_like(gt_results),
                    torch.zeros_like(gt_results),
                )[:, None, :, :].float()

                gt_results = F.conv2d(
                    gt_results,
                    torch.full([1, 1, args.tile_size, args.tile_size], 1.0),
