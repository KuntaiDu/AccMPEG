...
for mask_weight, mask_p, cont_weight, cont_p, base, tile in product(mask_weight_list, mask_p_list, cont_weight_list, cont_p_list, base_list, tile_list):
    ...
    os.system(f'CUDA_VISIBLE_DEVICES=3 python compress.py -i trafficcam_{base}.mp4 trafficcam_24.mp4 -s trafficcam.mp4 -o {output} --mask_weight {mask_weight} --mask_p {mask_p} --cont_weight {cont_weight} --cont_p {cont_p} --learning_rate 0.08 --num_iterations 100 --tile_size {tile} -g trafficcam_24.mp4')
