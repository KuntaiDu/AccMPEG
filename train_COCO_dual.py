...
class COCO_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"hq": transformhq(image), "lq": transformlq(image), "fid": idx}
...
def main(args):
    ...
    # initialize logger
    logger = logging.getLogger("train_COCO_dual")
    ...
    if len(glob.glob(args.ground_truth + "*")) != 0:
        ...
        for data in loader:
            ...
            # get salinecy
            gt_results = application.inference(hq_image)
            loss = application.calc_loss(lq_image, gt_results, args, train=True)
            loss.backward()
            lq_grad = lq_image.grad.norm(dim=1, p=2, keepdim=True)
            lq_grad = F.conv2d(
                lq_grad,
            ...
            # save it
            saliency[fid] = lq_grad.detach().cpu()
            ...
            if fid % 500 == 0:
                ...
                if args.visualize:
                    ...
                    lq_image = T.ToPILImage()(data["lq"][0])
                    lq_image = T.Resize(sizehq)(lq_image)
                    application.plot_results_on(
                        gt_results[0], lq_image, "Azure", args, train=True
                    )
                    lq_image = T.Resize(sizelq)(lq_image)
    ...
    for iteration in range(args.num_iterations):
        ...
        for idx, data in enumerate(training_loader):
            ...
            if any(fid not in saliency for fid in fids):
                continue
            target = torch.cat(
                [norm(log(F.interpolate(saliency[fid], size=(45, 80)))) for fid in fids]
            ...
            if any(fid % 500 == 0 for fid in fids):
                ...
                if args.visualize:
                    ...
                    target = sum(
                        (norm(target) > thresh).float() for thresh in thresh_list
                    )

                    visualize_heat(
                        image,
                        target.cpu().detach(),
                        f"train/{args.path}/{fid}_gt.png",
                        args,
                    )
                    # application.plot_results_on(
