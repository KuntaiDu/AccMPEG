    ...
    for iteration in range(args.num_iterations):
        ...
        for application in application_bundle:
            ...
            for fid, (video_slices, mask_slice) in enumerate(zip(zip(*[video.split(1) for video in videos]), mask.split(1))):
                ...
                # calculate the loss
                loss = application.calc_diff_acc(masked_image.cuda(), ground_truth_results[fid], args)
                if loss < 1.0:
                    loss.backward()
                    loss = loss.item()
                ...
                if fid % 20 == 0 and iteration % 5 == 4:
                    ...
                    plt.savefig(f'visualize/{args.output}_{fid}_attn.png', bbox_inches='tight')
        ...
        logger.info('Loss: %0.3f, Mask max: %.3f, min: %.3f, mean: %.3f, std: %.3f' % (loss, mask.max().item(), mask.min().item(), mask.mean().item(), mask.std().item()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learning_rate', type=float, help='The learning rate.', default=0.07)
