...
def main(args):
    ...
    for temp in range(1):
        ...
        for fid, (video_slices, mask_slice) in enumerate(
            ...
            regions = application.region_proposal(lq_image)[0]
            regions = center_size(regions).cpu()
            inference_results = application.inference(hq_image.cuda(), detach=True)[0]
            _, _, boxes, _ = application.filter_results(
                inference_results, args.confidence_threshold
            )
            boxes = center_size(boxes).cpu()
            sizes = regions[:, 2] * regions[:, 3]
            regions[sizes > (720 * 1280 * 0.04), :] = 0
