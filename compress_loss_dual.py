...
def main(args):
    ...
    for temp in range(1):
        ...
        for fid, (video_slices, mask_slice) in enumerate(
            ...
            lq_image, hq_image = video_slices[0].cuda(), video_slices[1].cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--upper_bound", type=float, help="The bound for the mask.", required=True,
    )
    parser.add_argument(
        "--lower_bound", type=float, help="The bound for the mask.", required=True,
    )
    parser.add_argument(
