...
for v, qp in product(v_list, qp_list):
    ...
    output = f"{v}_compressed_blackgen_dual_obj_qp_{qp}.mp4"

    if True or not os.path.exists(orig):
        os.system(
            f"python compress_object.py -i {v}_qp_30.mp4 -g {v}_qp_30.mp4 "
            f"-s {v} -o {orig} --qp {high} --app {app_name} --visualize_step_size 25"
        )
    ...
    os.system(
        f"python inference.py -i {output} --app {app_name} --visualize_step_size 25"
    )
