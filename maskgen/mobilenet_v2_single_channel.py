    ...
    def forward(self, x):
        ...
        if self.connect:
            ...
            for layer in self.features:
                ...
                if isinstance(layer, InvertedResidual) and layer.stride != 1:

                    ret.append(
                        nn.MaxPool2d(kernel_size=factor, stride=factor)(x)
                    )

                    assert factor % layer.stride == 0
                    factor = factor // layer.stride
