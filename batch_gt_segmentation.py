...
high = 30
tile = 16
model_name = "COCO_full_normalizedsaliency_vgg11_crossthresh_fcn_resnet101"
conv_list = [3, 11, 19]
# conv_list = [3]
bound_list = [0.5]
...
for v, conv, bound in product(v_list, conv_list, bound_list):
    ...
    output = f"{v}_blackgen_gt_conv_{conv}.mp4"

    if not os.path.exists(output):
        # if True:
        os.system(
            f"python compress_gt_segmentation.py -i {v}_qp_{base}.mp4 "
            f" {v}_qp_{high}.mp4 -s {v} -o {output} --tile_size {tile}  -p maskgen_pths/{model_name}.pth.best"
            f" --conv_size {conv} --visualize True"
            f" -g {v}_qp_{high}.mp4 --bound {bound} --force_qp {high} --smooth_frames 1"
        )
        os.system(f"python inference_segmentation.py -i {output}")

    os.system(
        f"python examine_segmentation.py -i {output} -g {v}_qp_{high}.mp4 --gt_confidence_threshold 0.7 --confidence_threshold 0.7 --stats stats_segmentation"
    )
