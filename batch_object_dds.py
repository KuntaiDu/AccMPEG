...
for v, (base, high), conv in product(v_list, base_high_list, conv_list):

    # output = f'{v}_compressed_ground_truth_2%_tile_16.mp4'
    orig = f"{v}_compressed_blackgen_ddsseg3_conv_{conv}_qp_{high}.mp4"
    ...
    if not os.path.exists(orig):
        ...
        os.system(
            f"python compress_object_segmentation.py -i {v}_qp_30.mp4 -g {v}_qp_{base}.mp4 "
            f"-s {v} -o {orig} --qp {high} --app {app_name} --conv_size {conv}"
        )

    if dual:

        output = f"{v}_compressed_blackgen_dual_ddsseg3_conv_{conv}_qp_{base}_{high}.mp4"
    ...
    os.system(
        f"python examine.py -i {orig} -g {v}_qp_{high}.mp4 --gt_confidence_threshold 0.7 --confidence_threshold 0.7 --stats stats_fcn50_measurement_new --app {app_name}"
    )
